import React, { useState, useEffect } from 'react';
import styles from './Sudoku.module.css';
import Help from './HelpSudoku/Help';


const SudokuGame = (props) => {
  const [grid, setGrid] = useState([]);
  const [initialGrid, setInitialGrid] = useState([]);
  const [selectedCell, setSelectedCell] = useState(null);
  const [mistakes, setMistakes] = useState(0);
  const [isComplete, setIsComplete] = useState(false);
  const [isGameOver, setGameOver] = useState(false);
  const [puzzleIndex, setPuzzleIndex] = useState(0);
  // const [gameState, setGameState] = useState(true);
  let INITIAL_PUZZLES = props.state.puzzles;
  let PUZZLE_SOLUTIONS = props.state.solutions;
  // тнициализация игры
  useEffect(() => {
    startNewGame();
  }, []);

  const startNewGame = () => {
    // Выбираем случайный пазл или следующий по кругу
    const newPuzzleIndex = (puzzleIndex + 1) % INITIAL_PUZZLES.length;
    setPuzzleIndex(newPuzzleIndex);
    const puzzle = INITIAL_PUZZLES[newPuzzleIndex].puzzle.map(row => [...row]);
    const initial = INITIAL_PUZZLES[newPuzzleIndex].puzzle.map(row => [...row]);

    setGrid(puzzle);
    setInitialGrid(initial);
    setSelectedCell(null);
    setMistakes(0);
    setIsComplete(false);
    setGameOver(false);
    //setGameState(true);
  };

  const handleCellClick = (row, col) => {
    if (initialGrid[row][col] === 0) {
      setSelectedCell({ row, col });
    }
  };

  const handleNumberInput = (number) => {
    if (!selectedCell || isComplete) return;

    const { row, col } = selectedCell;

    if (initialGrid[row][col] !== 0) return;

    const correctNumber = PUZZLE_SOLUTIONS[puzzleIndex].puzzle[row][col];
    const isCorrect = number === correctNumber;

    if (!isCorrect) {
      setMistakes(prev => prev + 1);
      if (mistakes + 1 >= 3) {
        alert('Слишком много ошибок! В будущем поле будет отображать недостающие цифры на сетке, но пока так');
        // setGameState(false);
        setGameOver(true);
        return;
      }
    }

    // Обновляем сетку
    const newGrid = [...grid];
    newGrid[row][col] = number;
    setGrid(newGrid);

    // Проверяем, заполнена ли вся сетка
    checkCompletion(newGrid);
  };

  const checkCompletion = (currentGrid) => {
    for (let i = 0; i < 6; i++) {
      for (let j = 0; j < 6; j++) {
        if (currentGrid[i][j] === 0 || currentGrid[i][j] !== PUZZLE_SOLUTIONS[puzzleIndex].puzzle[i][j]) return;
      }
    }
    setIsComplete(true);
  };

  const clearCell = () => {
    if (!selectedCell || isComplete) return;

    const { row, col } = selectedCell;
    if (initialGrid[row][col] === 0) {
      const newGrid = [...grid];
      newGrid[row][col] = 0;
      setGrid(newGrid);
      setIsComplete(false);
    }
  };

  const getCellClass = (row, col) => {
    const classes = [styles.cell];

    // Добавляем границы для блоков 2x3
    if (row % 2 === 0) classes.push(styles.topBlockBorder);
    if (col % 3 === 0) classes.push(styles.leftBlockBorder);

    // Выделяем выбранную ячейку
    if (selectedCell && selectedCell.row === row && selectedCell.col === col) {
      classes.push(styles.selected);
    }

    
    // Разные стили для начальных чисел
    if (initialGrid[row] && initialGrid[row][col] !== 0) {
      classes.push(styles.initial);
    }
    
    // Подсветка строки и столбца выбранной ячейки
    if (selectedCell && (selectedCell.row === row || selectedCell.col === col)) {
      classes.push(styles.highlighted);
    }
    if (PUZZLE_SOLUTIONS[puzzleIndex].puzzle[row][col] !== grid[row][col] && grid[row][col] !== 0) {
      classes.push(styles.wrong);
    }
    
    return classes.join(' ');
  };

  const renderGrid = () => {
   
    return grid.map((row, rowIndex) => (
      <div key={rowIndex} className={styles.row}>
        {row.map((cell, colIndex) => (
          <div
            key={`${rowIndex}-${colIndex}`}
            className={getCellClass(rowIndex, colIndex)}
            onClick={() => handleCellClick(rowIndex, colIndex)}
          >
            {cell !== 0 ? cell : ''}
          </div>
        ))}
      </div>
    ));
  };

  return (

    <div className={styles.container}>
      <div className={styles.header}>
        <h1 className={styles.title}>Судоку 6x6</h1>
        <div className={styles.stats}>
                <div className={styles.statItem}>Ошибки: {mistakes}/3</div>
                <div className={styles.statItem}>
                  Статус: {isComplete ? 'Завершено!' : 'В процессе'}
                </div>
              </div>
      </div>

      <div className={styles.gameArea}>
        <div className={styles.gridContainer}>
          <div className={styles.grid}>{renderGrid()}</div>
        </div>

        <div className={styles.controls}>
          <div className={styles.numberPad}>
            {
              [1, 2, 3, 4, 5, 6].map(num => (
                <button
                  key={num}
                  className={styles.numberButton}
                  onClick={() => handleNumberInput(num)}
                  disabled={!selectedCell || isComplete || isGameOver}
                >
                  {num}
                </button>
              ))
              //:
              //(
            }
            <div>
            </div>
          </div>

          <div className={styles.actionButtons}>
            <button
              className={styles.actionButton}
              onClick={clearCell}
              disabled={!selectedCell || isComplete}
            >
              Очистить
            </button>


          </div>

          <Help />

        </div>
      </div>



      {isComplete && (
        <div className={styles.completionMessage}>
          Поздравляем! Вы решили эту задачу!
        </div>
      )}
    </div>
  );
};

export default SudokuGame;